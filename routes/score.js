...
router.post('/getMatchScoreT', async (ctx)=>{
    let req = ctx.request.body;
    let {
        code,
        token
    ...
    if (err) {
        ...
        await db.find(query).then(async result => {
            ...
            try {
                ...
                if(flag===true){
                    let matchID = result1[0].id
                    let query3 = `select b.id,ruleName,projectID,score,ruleID from Rule a join (select * from MatchData where projectID in (select id from Project where matchID="${matchID}")) b on b.ruleID = a.id;`
                    let res3 = await db.find(query3)
                    for (const item of res3) {
                        let query4 = `select playerName from Project where id="${item.projectID}"`
                        let res4 = await db.find(query4)
